...
def filter_a_share_universe(
    ...
    *,
    ...
    if include_growth_boards:
        allowed_prefixes = allowed_prefixes + ("300", "301", "688", "689")
    out = out[
        out["代码"].str.startswith(allowed_prefixes)
...
class MarketDataProvider:
    ...
    def get_mainboard_universe(self, as_of: str | None = None, top_n: int = 80) -> list[str]:
        try:
            ...
            if df is None or df.empty:
                return []
            return filter_a_share_universe(df, include_growth_boards=False, top_n=top_n)
        except Exception:
            pass
        try:
    ...
    def get_all_market_universe(self, as_of: str | None = None, top_n: int = 80) -> list[str]:
        try:
            df = ak.stock_zh_a_spot_em()
            if df is None or df.empty:
                return []
        ...
        try:
            ...
            session.trust_env = False
            codes = _generate_mainboard_codes()
            growth_codes: list[str] = []
            for prefix in ("300", "301", "688", "689"):
                start = int(prefix) * 1000
                end = start + 1000
                for value in range(start, end):
                    code = f"{value:06d}"
                    market = "sh" if code.startswith(("688", "689")) else "sz"
                    growth_codes.append(f"{market}{code}")
            codes.extend(growth_codes)
            batches = [codes[i : i + 600] for i in range(0, len(codes), 600)]
            rows: list[dict] = []
            ...
            if not rows:
                return []
            df = pd.DataFrame(rows)
            df["代码"] = df["code"].astype(str).str.replace("^(sh|sz)", "", regex=True).str.zfill(6)
            df["名称"] = df["name"].astype(str)
            df["成交额"] = pd.to_numeric(df.get("amount"), errors="coerce")
            return filter_a_share_universe(df, include_growth_boards=True, top_n=top_n)
        except Exception:
            return []
